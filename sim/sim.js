...
Test.getRtFault = params => {
	return _.times(_.random(0, 3), item => {
...
Test.getSysData = params => {
	return {
		"video": {
			"storageSpace": {
				"total": _.random(96, 966367641600),
				// "used": _.random(10000, 9663676416),
				"used": 0,
				"unused": 0
			},
			"channel": {
		...
		if (idx)
			return _.times(13, _idx => {
				return {
					...
					value: _.random(0, 1024*500*50)
				}
			});
		return _.times(13, _idx => {
			return {
				time: new Date().getTime() - (12-_idx)*5000,
				value: _.random(1024*500, 1024*1024*100)
			}
		});
	});
};
...
Test.getNodeList = params => {
	let total = _.random(2000, 20000);
	return {
		total: 2000,
		items: _.times(params.pageSize, idx => {
			let type = _.random(0, 1);
			...
			return {
				...
				type: type,
				allocatedSpace: {
					used: type ? _.random(0, 30): -1,
					all: _.random(0, 30)+30
				},
				allocatedCpu: {
					used: cpu,
