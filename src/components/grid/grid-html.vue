<template>
	<div class="grid-html">
		{{params.used == -1 ? '-' : ''}}
		<gridProgress v-if="type == 'dropdown' && params.used != -1"
			:used="params.used"
			:all="params.all"
		></gridProgress>
		...
		<div v-if="type == 'node-status'" :class="{'higher': params == 0}">
			<div :class="{
				'icon-status-normal': params,
				'icon-status-fault': !params
			}"></div>
			{{params | getNodeStatus}}
		</div>
	</div>
</template>
<script>
...
<style lang="less">
	.grid-html{
		...
		.icon-status-normal, .icon-status-fault{
			vertical-align: middle;
		}
	}
</style>