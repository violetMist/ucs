	...
	<div class="broken-line">
		...
		<svg class="broken" viewBox="0 -35 570 160">
			...
			<g class="scope-bg">
				...
				<line v-for="(col, index) of cols" class="svg-col" :x1="8*col+60" y1="0" :x2="8*col+60" y2="160" :stroke= "showNo == index ? '#fff': '#eee'" :stroke-width= "showNo == index ? 1: 0.2"></line>
				<text v-for="col of cols" :x="8*col+63" y="175" :dx="-col.toString().length*5" fill="#87ccff" style="font-size: 10px;">{{col}}</text>
				<text v-for="n of 5" :x="getX(n)" :y="165-(n-1)*40" fill="#87ccff" style="font-size: 10px;">{{rows[n-1]}}</text>
				<text x="50" y="-15" fill="#87ccff" style="font-size: 10px;">速率</text>
				<text x="550" y="175" fill="#87ccff" style="font-size: 10px;">{{unit}}</text>
			</g>
		...
		computed: {
			...
			rows () {
				if (this.max > 1000*1024*1024)
					return [0, '50M', '100M', '500M', (this.max/(1024*1024)).toFixed(0)+'M'];
				return [0, '50M', '100M', '500M', '1000M'];
			},
			cols () {
		...
		methods: {
			...
			getY (value) {
				if (value >= 0 && value <= 50*1024*1024)
					return value / (50*1024*1024) * 40;
				if (value > 50*1024*1024 && value <= 100*1024*1024 )
					return (value-50*1024*1024) / (100*1024*1024-50*1024*1024) * 40 + 40;
				if (value > 100*1024*1024 && value <= 500*1024*1024)
					return (value-100*1024*1024) / (500*1024*1024-100*1024*1024) * 40 + 80;
				...
				if (value > 500*1024*1024 && value <= 1024*1024*1000)
					return (value-500*1024*1024) / (1000*1024*1024-500*1024*1024) * 30 + 120;
			...
			// 	if (value > 10*1024 && value <= 500*1024 )
