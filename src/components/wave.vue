		...
		methods: {
			...
			update (newUsed, newAll) {
				...
				let vm = this;
			      ...
			      new TWEEN.Tween({ tweenPercent: vm.percent, tweenHeight: vm.percent })
					.easing(TWEEN.Easing.Quadratic.Out)
					.onUpdate(function () { 
			...
			init () {
				...
				// 		self.x2 = -120;
				// 	self.timer = window.requestAnimationFrame(doAnimation);
				// }
				// // Start animation
				// self.timer = window.requestAnimationFrame(doAnimation);
				this.timer = setInterval(() => {
					this.x1 += 2.5;
					this.x2 += 2.5;
					if (this.x1 > 120 || this.x1 == 120)
						this.x1 = -120;
					if (this.x2 > 120 || this.x2 == 120)
						this.x2 = -120;
				}, 40);
			}
		},
