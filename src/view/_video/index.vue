	...
	>
		<div class="dialog-content">
			<div class="d-row">
				...
				<input type="password" name="access" v-model="accessValue" v-on:keyup.enter="checkAccess ('ok')">
			</div>
	...
	>
		<div class="dialog-content">
			...
			<div :class="{'d-row': true, 'disable': !rowDisable}">
				...
				<input type="number" min="0" max="3000" v-model.number="dialogData.day">
				<span>&nbsp;天</span>
				<Dropdown
					Key="hour"
			...
			<div class="hint re">请输入有效的录像覆盖周期！</div>
		</div>
	</diy-dialog>
</div>
</template>
<script>
	...
	export default {
		...
		methods: {
			...
			checkAccess (key) {
				if (key == 'ok') {
					UCS.Common('checkAccess', {access: md5(this.accessValue)}, result => {
						if (!result)
			...
			checkDialogFn (key) {
				if (key == 'channel') {
					...
					if (cycleType == 1 && ((!day && !hour) || (!_.isNumber(day) || !_.isNumber(hour) || day < 0 || day > 3000))) 
		...
		watch: {
			'dialogData.day': {
				handler (val, oldVal) {
					if (!_.isNull(val) && (val > 3000 || val < 0 || !_.isNumber(val))) {
						this.checkDialogFn('ok');
					} else if (!_.isNull(val)  && (val <= 3000 && val >= 0 )) {
						this.dialogClz = 'edit-video show';
					}
				}
			},
			'dialogData.hour': {
				handler (val, oldVal) {
					if (val !== oldVal
						&& this.dialogClz == 'edit-video recycle'
						&& this.dialogData.day <= 3000
						&& this.dialogData.day >= 0) {
						this.dialogClz = 'edit-video show';
					}
				}
			}
