	...
	>
		<div class="dialog-content">
			...
			<div class="d-row">
				...
				<input type="number" min="0" max='100' v-model.number="popObj.size">
				<span>&nbsp;TB</span>
			</div>
			<div class="d-row">
...
<script>
	...
	export default {
		...
		computed: {
			...
			heapWidth () {
				return (Math.floor((this.total+2)/3) + 1) * 987 + 'px';
			},
			detail36Type () {
		...
		methods: {
			...
			checkInvalid () {
				...
				if (_.isUndefined(this.popObj.name) || this.popObj.name.length == 0) {
					...
					this.showHint = true;
				} else if (_.isUndefined(this.popObj.size) || !Number(this.popObj.size) || this.popObj.size <= 0 || this.popObj.size > 100) {
